            ...
            try:
                with open(manifest_path) as manifest:
                    ...
                    for proto_type in manifest_json["protocolBuffers"]:
                        self.protocolBuffers_old[proto_type["fullname"]]=FlatType(proto_type["fullname"], proto_type["protoFile"] ,proto_type["version"])
    ...
    def update(self, flat_types):
        for flat_type in flat_types:
            if flat_type.name not in self.protocolBuffers_old:
                ...
                self.protocolBuffers[flat_type.name]=FlatType(flat_type.name,flat_type.proto_file,self.protocolBuffers_old[flat_type.name].version)
         ...
         try:
            with open(self.manifest_path, "w") as manifest_file:
                ...
                for proto_type in self.protocolBuffers.values():
                    protos.append({"fullname":proto_type.name,"version":proto_type.version,"protoFile": proto_type.proto_file })
